    ...
    <nav :style="`--navColor: ${navColor}`">
      <ul>
        <li class="logo"><button type="button" @click="gotoSection" data-section-color="light">Home</button>
          ...
          <svg viewBox="0 0 24 24" class="icon_up">
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        if (entry.isIntersecting) {
          if (sectionId !== 'intro') {
            document.querySelector('nav ul li.logo').classList.add('toup');
          } else {
            document.querySelector('nav ul li.logo').classList.remove('toup');
          }
